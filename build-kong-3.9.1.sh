...
# =============================================================================
#  build-kong-3.9.1.sh
#  Build Kong OSS 3.9.1 using SuriyaRuk/kong-build-tools
#  OpenResty: 1.29.2.1   |   Base OS: Ubuntu 20.04
# =============================================================================
set -euo pipefail
...
BUILD_TOOLS_DIR="/tmp/kong-build-tools-391"
KONG_SOURCE_DIR="/tmp/kong-source-391"
...
DOCKER_REPOSITORY="kong-build-local"
...
# ubuntu:20.04 base + fpm gem so no Docker Hub auth is needed.
if ! docker image inspect kong/fpm:0.5.1 &>/dev/null; then
  log "kong/fpm:0.5.1 not found locally — building a compatible FPM image ..."
...
FPMEOF
  log "  -> kong/fpm:0.5.1 built successfully"
else
  log "kong/fpm:0.5.1 already present locally — skipping build"
fi
...
# Legacy `docker build` (DOCKER_BUILDKIT=0); no buildx, no --load needed.
# Note: --progress is a BuildKit-only flag, legacy builder does not support it.
LOCAL_DOCKER_CMD="docker build"
